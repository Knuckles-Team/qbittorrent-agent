...
from qbittorrent_agent.qbittorrent_models import (
    TorrentInfo,
    TorrentProperties,
    Category,
    TransferInfo,
...
class QbittorrentApi:
        ...
        try:
            ...
            if response.status_code == 200:
                if "SID" in self.session.cookies:
                    ...
                    raise AuthError("Login successful but SID cookie not found in response.")
            elif response.status_code == 403:
                raise AuthError("User's IP is banned for too many failed login attempts.")
            else:
                raise AuthError(f"Login failed with status code {response.status_code}: {response.text}")
    ...
    def _handle_errors(self, response: requests.Response):
        if response.status_code == 401:
            ...
            raise UnauthorizedError("Forbidden: You don't have permission to access this resource.")
    ...
    @require_auth
    def get_torrents(self, filter: Optional[str] = None, category: Optional[str] = None, tag: Optional[str] = None) -> List[Dict]:
        """Get torrent list."""
        params = {}
    ...
    def delete_torrents(self, hashes: str, delete_files: bool = False):
        """Delete torrents."""
        return self._post("torrents/delete", data={"hashes": hashes, "deleteFiles": str(delete_files).lower()})

    @require_auth
    def add_torrent(self, urls: Optional[str] = None, torrent_files: Optional[List[str]] = None, **kwargs):
        """Add new torrent."""
        data = {}
        if urls:
            data["urls"] = urls
    ...
    @require_auth
    def search_start(self, pattern: str, plugins: str = "all", category: str = "all") -> Dict:
        """Start search."""
        return self._post("search/start", data={"pattern": pattern, "plugins": plugins, "category": category})

    @require_auth
    def search_status(self, search_id: Optional[int] = None) -> List[Dict]:
        """Get search status."""
        params = {}
        if search_id is not None: params["id"] = search_id
        return self._get("search/status", params=params)

    @require_auth
    def search_results(self, search_id: int, limit: int = 10, offset: int = 0) -> Dict:
        """Get search results."""
        return self._get("search/results", params={"id": search_id, "limit": limit, "offset": offset})
