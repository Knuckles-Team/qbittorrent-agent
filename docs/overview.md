# qbittorrent-agent — Concept Overview
...
```json
{
  "mcpServers": {
    "qbittorrent-agent": {
      "command": "uv",
      "args": ["run", "--with", "qbittorrent-agent", "qbittorrent-mcp"],
      "env": {}
    }
  }
}
...
```bash
qbittorrent-mcp --transport streamable-http --port 8001
```
